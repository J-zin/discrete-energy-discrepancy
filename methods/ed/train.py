...
from methods.ed.models.model import MLPScore, EBM
from methods.ed.models.loss import compute_loss, compute_transition_matrix
...
class Trainer:
    def __init__(self, model, train_iter, lr, weight_decay, steps, model_save_path, device=torch.device('cuda:1'), args=None):
        ...
        self.qt_matrix = compute_transition_matrix(args, t_noise=args.tnoise)
    ...
    def _run_step(self, x):
        ...
        loss = compute_loss(self.model, x, self.qt_matrix, self.args)
...
def train(
    ...
    # Not important if train or test?
    K = np.array(dataset.get_category_sizes())
    cat_types = dataset.get_category_types()
    
    num_numerical_features = dataset.X_num['train'].shape[1] if dataset.X_num is not None else 0
    print(K)
    print(cat_types)
    print(num_numerical_features)
    print(f'cat_tnoise: {cat_tnoise}')
