...
def perturb_categorical(samples, num_classes, t = 0.1, m_particles = 32):
    device = samples.device
    bs, C = samples.shape
    num_classes = torch.tensor(num_classes, device=device)
...
def compute_loss(energy_net, samples, args, epsilon = 1., m_particles = 32, w_stable = 1.):
    ...
    pert_x_cat = perturb_categorical(x_cat, args.num_classes, t = 0.1, m_particles = m_particles)
